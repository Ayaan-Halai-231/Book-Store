...
function AllOrders() {
  ...
  useEffect(() => {
    const fetch = async () =>{
      const response = await axios.get(
        'https://book-store-9-2ews.onrender.com/api/v1/get-all-orders',
        { headers }
        )
  ...
  const submitChanges = async (i) =>{
    ...
    const response = await axios.put(
      `https://book-store-9-2ews.onrender.com/api/v1/update-status/${id}`,
      Values,
      { headers }
    );
          ...
          <div key={i} className="bg-zinc-800 w-full rounded py-2 px-4 flex gap-2 hover:bg-zinc-900 hover:cursor-pointer transition-all duration">
            ...
            <div className="w-[30%] md:w-[16%]">
              <h1 className="font-semibold">
                <button className="hover:slice-105 transition-all duration-300" 
