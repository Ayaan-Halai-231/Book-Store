  ...
  return (
    <>
      ...
      <div className={`${MobileNav} bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`}>
        {links.map((items,i)=>(
          <Link to={items.link} className="text-white text-4xl mb-4 hover:text-blue-500 transition-all duration-300"
            key={i} 
            onClick={()=>
              MobileNav === 'hidden' ? setMobileNav('block') : setMobileNav('hidden')
            }  
            ...
            {isLoggedIn === false && (
              ...
              <Link 
                to="/SignUp" 
                className={`${MobileNav} px-8 py-2 mb-8 text-3xl font-semiblod bg-blue-500 rounded hover:bg-white hover:text-zinc-800 transition-all duration-300`}>
                SignUp
              </Link>
              </>
            )}
